...
def tooClose(strand, strands, minDist):
    '''
    Computes the strandDistance between the strand and all strands
    in the list, and if any strands in the list are closer than minDist,
    this method returns true.
    '''
    for s in strands:
        if strandDistance(strand, s) < minDist:
                return True
...
def drawOrigin(strandLength):
    bases = ('A', 'C', 'G', 'T')
    return [random.choice(bases) for _ in range(strandLength)]
...
for i in range(0, numClusters):
    dna_strand_mean = drawOrigin(strandLength)
...
minClusterVar = 0
maxClusterVar = strandLength // 2 if strandLength >= 2 else 2
    ...
    for i in range(0, numClusters):
        ...
        for j in range(0, numStrands):
            ...
            for _ in range(random.choice(range(1 + variance))):
                i = random.randint(0, strandLength-1)
